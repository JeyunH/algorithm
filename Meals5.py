...
# [6] 현재 시간에 따른 식사유형 강조 (오늘 또는 내일(18시 이후)의 식단에 대해 강조)
def get_current_meal(target_date):
    now = datetime.now()
...
# [10] 타이틀 프레임
title_label = ttk.Label(root, text=f"📅 {selected_weekday}의 식단", style="Title.TLabel")
...
def update_ui(target_date):
    ...
    current_meal = get_current_meal(target_date)
    for meal in ["조식", "중식", "석식"]:
